...
const TagSelectComponent = ({ tags }: { tags: TagOnId[] }) => {
    const router = useRouter()

    const handleEditTag = (tag: TagOnId) => {
        SetLocalStrageTag(tag.name)
        SetLocalStrageId(tag.id)
        router.push("/tag-edit")
    }

    return (
        <div className=" w-full flex flex-col h-full  bg-white ">
                <ul className=" w-full space-y-3 py-6 ">
                    {tags.map((v: TagOnId, i: number) => (
